...
class DSCExperiment:
    ...
    def __init__(self, temp_data, signal_data, name="Eksperyment"):
        ...
        self.signal_processed = None # Będzie przechowywać dane w J/(mol·K)
    ...
    def process_scan(self, buffer_exp, min_temp, max_temp, protein_conc_mM, heating_rate_C_min):
        ...
        Wynikowy sygnał będzie w J/(mol·K), aby był zgodny z modelami.
        """
        ...
        # Konwersja sygnału do J/(mol·K)
        # Założenie: Sygnał surowy (signal_raw) jest w uW (mikrowatach)
        # uW = uJ/s
        # uJ/s / (K/s) = uJ/K
        signal_uJ_K = signal_trimmed / heating_rate_K_s #
        signal_uJ_Kmol = signal_uJ_K / moles #
        # Przeliczamy na J/(mol·K) zamiast kJ/(mol·K) aby pasowało do modeli
        signal_J_Kmol = signal_uJ_Kmol / 1e6 # (1e6 bo z mikroJ na J)

        self.temp_processed = temp_trimmed #
        self.signal_processed = signal_J_Kmol #
        self.results['concentration_mM'] = protein_conc_mM #
        self.results['heating_rate_C_min'] = heating_rate_C_min #
        print("Przetwarzanie skanu zakończone. Sygnał w J/(mol·K).")
    ...
    def analyze_baseline_vanthoff(self, pre_trans_range, post_trans_range):
        ...
        UWAGA: Ta metoda operuje na już przetworzonych danych (w J/(mol·K)).
        Entalpia kalorymetryczna będzie w J/mol, a dCp w J/(mol·K).
        """
        if self.temp_processed is None:
        ...
        # Jeśli Y jest w J/(mol·K), a T w °C (czyli dT w K), to trapz da J/mol
        delta_H_cal = np.trapz(Y_corrected, T) #

        half_point_idx = np.searchsorted(Bas, 0.5) #
        # dCp będzie w J/(mol·K), ponieważ P1 i P2 operują na Y w J/(mol·K)
        dCp = np.polyval(P2, T[half_point_idx]) - np.polyval(P1, T[half_point_idx]) #

        self.signal_processed = Y_corrected #
        self.results['delta_H_calorimetric_J_mol'] = delta_H_cal #
        self.results['delta_Cp_J_molK'] = dCp #
    ...
    def plot(self, plot_type='processed', **kwargs):
        ...
        if plot_type == 'raw':
            ...
            ylabel_text = 'Sygnał [uW]' # Zakładana jednostka dla surowych danych
            ...
            ylabel_text = 'Pojemność cieplna [J/(mol·K)]' #
